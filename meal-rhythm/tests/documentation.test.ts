...
it('启动说明和实施状态的本地链接均能找到对应文件',async()=>{
  for(const file of ['README.md','docs/implementation-status.md','docs/testing.md','docs/cloud-verification.md']) {
...
it('云端说明区分独立分支、模拟器验证与真机发行',async()=>{
  const guide=await readFile('docs/cloud-verification.md','utf8');
  for(const required of ['codex/meal-rhythm-ios','meal-rhythm/','main','XCTest','XCUITest','TestFlight','签名','503'])expect(guide).toContain(required);
});
